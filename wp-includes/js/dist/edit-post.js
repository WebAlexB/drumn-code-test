...
const getEditedPostTemplate = (0,external_wp_data_namespaceObject.createRegistrySelector)(select => () => {
  ...
  // post type template slug(ex page, single-post, single-product etc..).
...
function ManagePatternsMenuItem() {
  const url = (0,external_wp_data_namespaceObject.useSelect)(select => {
    ...
    // The site editor and template both check whether the user has
    // edit_theme_options capabilities. We can leverage that here and not
    // display the manage patterns link if the user can't access it.
    return canUser('create', 'template') ? patternsUrl : defaultUrl;
  }, []);
...
function Editor({
  ...
  const {
    ...
    const canViewTemplate = canUser('read', 'template');
    return {
