<?php
/**
 * Utilities used to fetch and create template and template parts.
 *
 * @package WordPress
 * @since 5.8.0
 */
 ...
 * block themes might be using block-template or block-template-parts,
 ...
 *
 *     @type string $wp_template      Theme-relative directory name for block template.
...
function get_default_block_template_types() {
	$default_template_types = array(
		...
		'front-page'     => array(
			...
			'description' => __( 'Displays your site\'s homepage, whether it is set to display latest posts or a static page. The Front Page template takes precedence over all template.' ),
		),
		'singular'       => array(
		...
		'archive'        => array(
			...
			'description' => __( 'Displays any archive, including posts by a single author, category, tag, taxonomy, custom post type, and date. This template will serve as a fallback when more specific template (e.g. Category or Tag) cannot be found.' ),
		),
		'author'         => array(
 ...
 * @param string $template_type Template type. Either 'wp_template' or 'wp_template_part'.
 * @param array  $query {
 *     Arguments to retrieve template. Optional, empty by default.
 *
 *     @type string[] $slug__in     List of slugs to include.
 *     @type string[] $slug__not_in List of slugs to skip.
 *     @type string   $area         A 'wp_template_part_area' taxonomy value to filter by (for 'wp_template_part' template type only).
 *     @type string   $post_type    Post type to get the template for.
 * }
 *
	...
	foreach ( $themes as $theme_slug => $theme_dir ) {
		...
		foreach ( $theme_template_files as $template_file ) {
			...
			if ( 'wp_template' === $template_type ) {
				...
				// The custom template with no associated post types are available for all post types.
 ...
 *
 * @param array  $query {
 *     Optional. Arguments to retrieve template.
 *
 *     @type string[] $slug__in  List of slugs to include.
 ...
 * }
 * @param string $template_type Template type. Either 'wp_template' or 'wp_template_part'.
 * @return WP_Block_Template[] Array of block template.
 */
function get_block_templates( $query = array(), $template_type = 'wp_template' ) {
	/**
	 * Filters the block template array before the query takes place.
	 *
	 * Return a non-null value to bypass the WordPress queries.
	 *
	 * @since 5.9.0
	 *
	 * @param WP_Block_Template[]|null $block_templates Return an array of block template to short-circuit the default query,
	 *                                                  or null to allow WP to run its normal queries.
	 * @param array  $query {
	 *     Arguments to retrieve template. All arguments are optional.
	 *
	 *     @type string[] $slug__in  List of slugs to include.
	 *     @type int      $wp_id     Post ID of customized template.
	 *     @type string   $area      A 'wp_template_part_area' taxonomy value to filter by (for 'wp_template_part' template type only).
	...
	// This is only needed for the regular template/template parts post type listing and editor.
	...
	if ( ! isset( $query['wp_id'] ) ) {
		/*
		 * If the query has found some use template, those have priority
	...
	/**
	 * Filters the array of queried block template array after they've been fetched.
	 *
	 * @since 5.9.0
	 *
	 * @param WP_Block_Template[] $query_result Array of found block template.
	 * @param array               $query {
	 *     Arguments to retrieve template. All arguments are optional.
	 *
	 *     @type string[] $slug__in  List of slugs to include.
 ...
 *
 * This is a fallback of get_block_template(), used when no template are found in the database.
 *
 * @since 5.9.0
 *
 * @param string $id            Template unique identifier (example: 'theme_slug//template_slug').
...
/**
 * Creates an export of the current template and
...
function wp_generate_block_templates_export_file() {
	...
	// Load template into the zip file.
...
function get_template_hierarchy( $slug, $is_custom = false, $template_prefix = '' ) {
	...
	// Most default template don't have `$template_prefix` assigned.
