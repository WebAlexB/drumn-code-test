...
class WP_REST_Templates_Controller extends WP_REST_Controller {
	 ...
	 *
	 * @since 6.1.0
	 * @since 6.3.0 Ignore empty template.
	 *
	...
	protected function permissions_check( $request ) {
		 ...
		 * This capability is required to edit/view/delete template.
		 */
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			return new WP_Error(
				'rest_cannot_manage_templates',
				__( 'Sorry, you are not allowed to access the template on this site.' ),
				array(
	 ...
	 * requires using a path like /wp/v2/template/twentytwentytwo//home. There
	...
	/**
	 * Checks if a given request has access to read template.
	 *
	 * @since 5.8.0
	 * @since 6.6.0 Allow users with edit_posts capability to read template.
	 *
	 * @param WP_REST_Request $request Full details about the request.
	 * @return true|WP_Error True if the request has read access, WP_Error object otherwise.
	 */
	 ...
	 *
	 * @since 5.8.0
	 * @since 6.6.0 Allow users with edit_posts capability to read individual template.
	 *
	 * @param WP_REST_Request $request Full details about the request.
	...
	public function get_item( $request ) {
		...
		if ( ! $template ) {
			return new WP_Error( 'rest_template_not_found', __( 'No template exist with that id.' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( ! count( $posts ) ) {
			return new WP_Error( 'rest_template_insert_error', __( 'No template exist with that id.' ), array( 'status' => 400 ) );
		}
		$id            = $posts[0]->id;
	...
	private static function get_wp_templates_original_source_field( $template_object ) {
		if ( 'wp_template' === $template_object->type || 'wp_template_part' === $template_object->type ) {
			...
			// older customized template by checking for no origin and a 'theme'
			// or 'custom' source.
			...
			// was only added to template in WordPress 5.9. Fallback to showing the
			// site logo and title.
	...
	public function get_collection_params() {
		return array(
			...
			'post_type' => array(
				'description' => __( 'Post type to get the template for.' ),
				'type'        => 'string',
			),
		);
	}
