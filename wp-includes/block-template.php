...
function _add_template_loader_filters() {
	if ( isset( $_GET['_wp-find-template'] ) && current_theme_supports( 'block-template' ) ) {
...
function locate_block_template( $template, $type, array $templates ) {
	...
	if ( ! current_theme_supports( 'block-template' ) ) {
		return $template;
	}
	...
	if ( $template ) {
		...
		// Locate the index of $template (without the theme directory path) in $template.
		...
		// we will only consider block template with higher or equal specificity.
	...
	if ( $block_template ) {
		...
		if ( 'index' === $type ) {
			...
			return ''; // So that the template loader keeps looking for template.
		}
	}
...
function resolve_block_template( $template_type, $template_hierarchy, $fallback_template ) {
	...
	// Find all potential template 'wp_template' post matching the hierarchy.
	$query     = array(
		'slug__in' => $slugs,
	);
	$templates = get_block_templates( $query );

	// Order these template per slug priority.
	...
	if (
		...
		if (
			...
			// Unfortunately, we cannot trust $template[0]->theme, since it will always
...
function get_the_block_template_html() {
	...
	/*
	 * Most block themes omit the `core/query` and `core/post-template` blocks in their singular content template.
	 * While this technically still works since singular content template are always for only one post, it results in
	 * the main query loop never being entered which causes bugs in core and the plugin ecosystem.
	 *
	 * The workaround below ensures that the loop is started even for those singular template. The while loop will by
